...
import rospy
# from beginner_tutorials import SensorInfo, BoxH
from beginner_tutorials.msg import SensorInfo, BoxH
...
def sensor_info_callback(data, bhi_pub):
    ...
    if height_box > 10:
        ...
        box_height_info = BoxH()
        # Update height of box.
        box_height_info.box_height = data.sensor_data.range()
...
if __name__ == '__main__':
    ...
    rospy.loginfo('Waiting for topic %s to be published...', "/box_height")
    rospy.wait_for_message("/box_height", SensorInfo)
    rospy.loginfo('%s topic is now available!',"/box_height")

    # Create the publisher for Part3 here
    bhi_publisher = rospy.Publisher('/box_height', SensorInfo, queue_size=10)
    ...
    rospy.Subscriber('/box_height', SensorInfo, sensor_info_callback, bhi_publisher)
